...
set.seed(100)
models <- list(
  NaiveBayes = train(win ~ ., data = tictac_train, method = "nb"),        #Naive Bayes - nb
  #DecisionTree = train(win ~ ., data = tictac_train, method = "dt"),     #Decision Tree - dt
  NeuralNetwork = train(win ~ ., data = tictac_train, method = "nnet"),   #Neural Network - nn
  NearestNeighbour = train(win ~ ., data = tictac_train, method = "knn"), #Nearest Neighbour - knn
  SVM = train(win ~ ., data = tictac_train, method = "svmLinear")         #SVM (linear kernel) - svm
)
